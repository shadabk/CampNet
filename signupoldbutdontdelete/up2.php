	...
	ini_set("sendmail_from", "shadabk96@gmail.com");
		...
		if(!empty($username)) {
			if(strcmp($password, $password2)) {
				...
				if(!($row = mysqli_fetch_array($query))){
					...
					$query = "INSERT INTO tempsignupinfo (timestamp, mis, name, college, branch, year, username, emailID, password, ads_count, passkey) VALUES ($timestamp, $mis, '$fname', '$college', '$branch', $year, '$username', '$email', '$password', $ads_count, '$confirm_code')";
					$data = mysqli_query($db, $query)or die(mysqli_error());
					if($data)
					{
						$to=$email;
						$subject="Your CampNet confirmation link here";
						$header="from: shadabk96@gmail.com ";
						$message="Your Comfirmation link \r\n";
						$message.="Click on this link to activate your account \r\n";
						$message.="http://campnet.pe.hu/CampNet/signup/confirm.php?passkey=$confirm_code";
